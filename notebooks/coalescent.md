...
for i, layer in enumerate(relationships):
    for j, parents in enumerate(layer):
        for sign in [-1, 1]:
            ...
            patch = mpl.patches.PathPatch(path, edgecolor="k", facecolor="none")
            ax.add_patch(patch)
...
metadata = {
    ...
    "CoalescentNeuralConfig-CoalescentMixtureDensityConfig.pkl": ("MDN-compressed\nABC", "s"),
    ...
    "CoalescentMinimumConditionalEntropyConfig.pkl": ("minimum entropy", "D"),
