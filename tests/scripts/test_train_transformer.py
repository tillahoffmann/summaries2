...
@pytest.mark.parametrize(
    "config", [x for x in TRAIN_CONFIGS if x.startswith("Benchmark")]
)
...
@pytest.mark.parametrize(
    "config", [x for x in TRAIN_CONFIGS if x.startswith("Coalescent")]
)
def test_train_transformer_coalescent(config: str, tmp_path: Path) -> None:
    ...
    __main__preprocess_coalescent(
        map(str, [coaloracle, tmp_path, "train:98", "validation:2"])
    )
...
def test_train_transformer_tree(config: str, tmp_path: Path) -> None:
    ...
    for path in [train_path, validation_path]:
        __main__simulate_data(
            ["TreeSimulationConfig", "--n-observations=13", "--n-samples=10", str(path)]
        )
