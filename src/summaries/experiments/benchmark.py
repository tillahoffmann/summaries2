...
def simulate_benchmark(
    ...
    data = random_state.normal(
        u, np.sqrt(1 - u**2), (*params.shape[:-1], n_observations)
    )
    data *= 2 * random_state.binomial(1, 0.5, data.shape) - 1
    return np.concatenate(
        [data[..., None], random_state.normal(0, 1, data.shape + (n_noise_features,))],
        axis=-1,
    )
...
class BenchmarkPosteriorMixtureDensityTransformer(BenchmarkPosteriorMeanTransformer):
    ...
    def forward(
        self, data: torch.Tensor, transformed: torch.Tensor | None = None
    ) -> torch.Tensor:
        ...
        scales: torch.Tensor = self.mixture_parameters["scales"](transformed).exp()[
            ..., None
        ]
        component_dist = distributions.Independent(
            distributions.Normal(locs, scales), 1
        )
