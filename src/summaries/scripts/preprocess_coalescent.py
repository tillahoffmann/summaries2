...
def __main__(argv: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        "splits",
        help="path to dataset splits and sizes as `[path]:[size]`",
        type=_parse_splits,
        nargs="+",
    )
    ...
    data = pd.read_csv(args.coaloracle).sample(
        frac=1, random_state=args.seed, replace=False
    )
    ...
    assert (size := sum(splits.values())) == data.shape[0], (
        f"Splits {splits} must add up to dataset size {data.shape[0]}; got {size}."
    )
